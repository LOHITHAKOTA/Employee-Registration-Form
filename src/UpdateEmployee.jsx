...
function UpdateEmployee() {
  ...
  useEffect(() => {
    setData(prev => ({
      ...prev,
      id: prev.id.toString(),
      salary: prev.salary?.toString(),
      exp: prev.exp?.toString()
    }));
  }, [state]);
  ...
  const handleSubmit = e => {
    e.preventDefault();
    axios.put('http://localhost:9999/projects/update', {
      ...data,
      id: Number(data.id),
  ...
  return (
   <div className="container">
      <aside className="sidebar">
        <ul>
          <li onClick={() => navigate('/add')}>View</li>
          <li onClick={() => navigate('/update')}>Add</li>
        </ul>
      </aside>
